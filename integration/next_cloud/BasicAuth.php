...
class BasicAuth extends Base {
	...
	public function checkPassword($uid, $password) {
		...
		$context = stream_context_create(array(
			'http' => array(
				'method' => "GET",
				'header' => "authorization: Basic " . base64_encode("$uid:default_auth_gui:$password"),
				'follow_location' => 0
			))
		);
		...
		$body = file_get_contents($this->authUrl, false, $context);
		$body_obj = json_decode($body, true);

		// Set user groups
		$groups = [];
		$groups[] = $body_obj["data"]["group"]["name"];
